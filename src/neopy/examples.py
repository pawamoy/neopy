...
def example2():
    ...
    query = graph.match(you).create(you, like, neo).return_(you, like, neo)
...
def example3_naive_python():
    ...
    for name in names:
        queries.append(graph.match(you).create(you, friend, Person(name=name)).return_(you))
...
def example3_foreach():
    ...
    query = graph.match(you).foreach(name_variable, names, graph.create(you, friend, Person(name=name_variable)))
    return get_result(query)
...
def example4():
    ...
    query = graph.match(N("you", name="You"), RelTo(T("friend")), N("yourFriends")).return_("you", "yourFriends")
    return get_result(query)
...
def example5():
    ...
    query = (
        graph.match(neo)
        .match(anna)
        .create(anna, RelTo(T("friend")), N(person, name="Amanda"), RelTo(T("worked_with")), neo)
    )
    return get_result(query)
...
def example6_composed():
    ...
    query = graph.match(you.set(name="You")).match(expert, worked_with, db).match(path).return_(db, expert, path)
...
def example7():
    ...
    query = (
        graph.optional_match(N("user", L("User")), Rel(T("friends_with")), N("friend", L("User")))
        .where(user__id=1234)
        .return_("user", number_of_friends=Count("friend"))
    )
    ...
    query = graph.optional_match(user, rel, friend).where(user__id=1234).return_(user, n_of_f)
